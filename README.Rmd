...
```{r ct2013, message=FALSE, cache=TRUE}
...
The aforementioned `geoid` parameter allows the user to mix different levels of geography when specifying a reference population. Employing 2000 decennial census data, the code below stores the ADIs of the census tracts in every county entirely or partially on the Delmarva peninsula:
...
Furthermore, the gray borders between census tracts are a bit thick and in many cases totally obscure some of the smaller tracts. Borders can be removed by setting `lwd = 0` within the `geom_sf()` function.
...
```{r chautauqua, cache=TRUE, message=FALSE}
chautauqua <- 
  get_adi(
    "tract",
    state = "New York",
